...
goog.ui.Control.IeMouseEventSequenceSimulator_.prototype.handleClick_ = function(a) {
    if (this.clickExpected_) this.clickExpected_ = !1;
        ...
        this.control_.handleMouseDown(new goog.events.BrowserEvent(b, a.currentTarget));
        b.type = goog.events.EventType.MOUSEUP;
        this.control_.handleMouseUp(new goog.events.BrowserEvent(b, a.currentTarget));
        b.button = c;
        b.type = d
    }
};
...
Blockly.Connection.prototype.connect = function(a) {
    ...
    if (this.type == Blockly.INPUT_VALUE || this.type == Blockly.OUTPUT_VALUE) {
        ...
        if (a.targetConnection) {
            var b = a.getShadowDom();
            a.setShadowDom(null);
            var c = a.targetBlock();
            c.setParent(null);
            if (c.isShadow()) a.setShadowDom(Blockly.Xml.blockToDom_(c)), c.dispose();
            else {
                if (!c.outputConnection) throw "Orphan block does not have an output connection.";
                for (var d = this.sourceBlock_, e; e = Blockly.Connection.singleConnection_(d, c);)
                    if (d = e.targetBlock(), !d || d.isShadow()) {
                        c.outputConnection.connect(e);
                        c = null;
                        break
                    }
                c && setTimeout(function() {
                    c.outputConnection.bumpAwayFrom_(a)
                }, Blockly.BUMP_DELAY);
                a.setShadowDom(b)
            }
        }
        ...
        if (a.targetConnection) {
            ...
            c = a.targetBlock();
            c.setParent(null);
            if (!c.previousConnection) throw "Orphan block does not have a previous connection.";
            for (d = this.sourceBlock_; d.nextConnection;)
                if (d.nextConnection.targetConnection) d = d.getNextBlock();
                else {
                    c.previousConnection.checkType_(d.nextConnection) && (d.nextConnection.connect(c.previousConnection), c = null);
                    break
                }
            c && setTimeout(function() {
                    c.previousConnection.bumpAwayFrom_(a)
                },
                Blockly.BUMP_DELAY)
        }
    }
    this.isSuperior() ? (b = this.sourceBlock_, d = a.sourceBlock_) : (b = a.sourceBlock_, d = this.sourceBlock_);
    this.targetConnection = a;
    a.targetConnection = this;
    d.setParent(b);
    b.rendered && b.updateDisabled();
    d.rendered && d.updateDisabled();
    b.rendered && d.rendered && (this.type == Blockly.NEXT_STATEMENT || this.type == Blockly.PREVIOUS_STATEMENT ? d.render() : b.render())
};
...
Blockly.Connection.prototype.disconnect = function() {
    ...
    var b, c;
    this.isSuperior() ? (b = this.sourceBlock_, c = a.sourceBlock_, a = this) : (b = a.sourceBlock_, c = this.sourceBlock_);
...
Blockly.Connection.prototype.highlight = function() {
    var a;
    this.type == Blockly.INPUT_VALUE || this.type == Blockly.OUTPUT_VALUE ? (a = this.sourceBlock_.RTL ? -Blockly.BlockSvg.TAB_WIDTH : Blockly.BlockSvg.TAB_WIDTH, a = "m 0,0 v 5 c 0,10 " + -a + ",-8 " + -a + ",7.5 s " + a + ",-2.5 " + a + ",7.5 v 5") : a = this.sourceBlock_.RTL ? "m 20,0 h -5 " + Blockly.BlockSvg.NOTCH_PATH_RIGHT + " h -5" : "m -20,0 h 5 " + Blockly.BlockSvg.NOTCH_PATH_LEFT + " h 5";
...
Blockly.Connection.prototype.closest = function(a, b, c) {
    function d(b) {
        b = e[b];
        if ((b.type == Blockly.OUTPUT_VALUE || b.type == Blockly.PREVIOUS_STATEMENT) && b.targetConnection || b.type == Blockly.INPUT_VALUE && b.targetConnection && !b.targetBlock().isMovable() && !b.targetBlock().isShadow() || !q.checkType_(b)) return !0;
...
Blockly.Xml.blockToDom_ = function(a) {
    ...
    Blockly.Realtime.isEnabled() && b.setAttribute("id", a.id);
    if (a.mutationToDom) {
    ...
    if (c = a.getCommentText()) c = goog.dom.createDom("comment",
        null, c), "object" == typeof a.comment && (c.setAttribute("pinned", a.comment.isVisible()), d = a.comment.getBubbleSize(), c.setAttribute("h", d.height), c.setAttribute("w", d.width)), b.appendChild(c);
    ...
    if (c = a.getNextBlock()) h = goog.dom.createDom("next", null, Blockly.Xml.blockToDom_(c)), b.appendChild(h);
...
Blockly.Xml.domToBlockHeadless_ = function(a, b, c) {
    ...
    for (var g = null, f = 0, h; h = b.childNodes[f]; f++)
        if (3 != h.nodeType) {
            for (var k = g = null, l = 0, q; q = h.childNodes[l]; l++) 1 == q.nodeType && ("block" == q.nodeName.toLowerCase() ?
                g = q : "shadow" == q.nodeName.toLowerCase() && (k = q));
            !g && k && (g = k);
            ...
            switch (h.nodeName.toLowerCase()) {
                ...
                case "comment":
                    ...
                    m && setTimeout(function() {
                        d.comment && d.comment.setVisible && d.comment.setVisible("true" == m)
                    }, 1);
                    g = parseInt(h.getAttribute("w"), 10);
                    h = parseInt(h.getAttribute("h"), 10);
                    !isNaN(g) && !isNaN(h) && d.comment && d.comment.setVisible && d.comment.setBubbleSize(g,
                        h);
                    break;
                case "data":
                ...
                case "field":
                    g = d.getField(l);
                    if (!g) {
                        console.warn("Ignoring non-existent field " + l + " in block " + e);
                        break
                    }
                    g.setValue(h.textContent);
                    break;
                case "value":
                case "statement":
                    h = d.getInput(l);
                    if (!h) {
                        console.warn("Ignoring non-existent input " + l + " in block " + e);
                        break
                    }
                    k && h.connection.setShadowDom(k);
                    if (g)
                        if (g = Blockly.Xml.domToBlockHeadless_(a, g, c), g.outputConnection) h.connection.connect(g.outputConnection);
                        else if (g.previousConnection) h.connection.connect(g.previousConnection);
                    else throw "Child block does not have output or previous statement.";
                    break;
                case "next":
                    k && d.nextConnection && d.nextConnection.setShadowDom(k);
                    if (g) {
    ...
    "shadow" == b.nodeName.toLowerCase() && d.setShadow(!0);
...
Blockly.WorkspaceSvg.prototype.addTopBlock = function(a) {
    ...
    Blockly.Realtime.isEnabled() && !this.options.parentWorkspace && Blockly.Realtime.addTopBlock(a)
};
Blockly.WorkspaceSvg.prototype.removeTopBlock = function(a) {
    Blockly.WorkspaceSvg.superClass_.removeTopBlock.call(this, a);
    Blockly.Realtime.isEnabled() && !this.options.parentWorkspace && Blockly.Realtime.removeTopBlock(a)
};
...
Blockly.WorkspaceSvg.prototype.isDeleteArea = function(a) {
    a = Blockly.mouseToSvg(a, Blockly.mainWorkspace.options.svg);
...
Blockly.WorkspaceSvg.prototype.onMouseWheel_ = function(a) {
    ...
    var b = 0 < a.deltaY ? -1 : 1,
        c = Blockly.mouseToSvg(a, this.options.svg);
    this.zoom(c.x, c.y, b);
    a.preventDefault()
};
...
Blockly.WorkspaceSvg.prototype.showContextMenu_ = function(a) {
    if (!this.options.readOnly) {
        var b = [],
            c = this.getTopBlocks(!0),
            d = {};
        d.text = Blockly.Msg.CLEAN_UP;
        d.enabled = 1 < c.length;
        d.callback = this.cleanUp_.bind(this);
        b.push(d);
        if (this.options.collapse) {
            for (var e = d = !1, f = 0; f < c.length; f++)
                for (var g = c[f]; g;) g.isCollapsed() ? d = !0 : e = !0, g = g.getNextBlock();
            var h = function(a) {
                    for (var b = 0, d = 0; d < c.length; d++)
                        for (var e = c[d]; e;) setTimeout(e.setCollapsed.bind(e, a), b), e = e.getNextBlock(), b += 10
                },
                e = {
                    enabled: e
                };
            e.text = Blockly.Msg.COLLAPSE_ALL;
            e.callback = function() {
                h(!0)
            };
            b.push(e);
...
Blockly.WorkspaceSvg.prototype.zoom = function(a, b, c) {
    var d = this.options.zoomOptions.scaleSpeed,
        e = this.getMetrics(),
        f = this.options.svg.createSVGPoint();
    f.x = a;
    f.y = b;
...
Blockly.Mutator.prototype.resizeBubble_ = function() {
    ...
    d += 3 * a;
    if (Math.abs(this.workspaceWidth_ - d) > a || Math.abs(this.workspaceHeight_ - b) > a) this.workspaceWidth_ = d, this.workspaceHeight_ = b, this.bubble_.setBubbleSize(d + a, b + a), this.svgDialog_.setAttribute("width", this.workspaceWidth_), this.svgDialog_.setAttribute("height",
...
Blockly.Mutator.prototype.setVisible = function(a) {
    if (a != this.isVisible())
        if (a) {
            ...
            this.resizeBubble_();
            Blockly.bindEvent_(this.workspace_.getCanvas(), "blocklyWorkspaceChange", this.block_,
                function() {
                    b.workspaceChanged_()
                });
            this.updateColour()
...
Blockly.Block.prototype.dispose = function(a, b, c) {
    ...
    Blockly.Realtime.isEnabled() && !Blockly.Realtime.withinSync &&
...
Blockly.ContextMenu.show = function(a, b, c) {
    ...
    if (b.length) {
        ...
        for (var e = 0, f; f = b[e]; e++) {
            ...
            f.enabled && goog.events.listen(g, goog.ui.Component.EventType.ACTION, function(a) {
                return function() {
                    Blockly.doCommand(a)
                }
            }(f.callback))
...
Blockly.BlockSvg.prototype.onMouseUp_ = function(a) {
    ...
    Blockly.doCommand(function() {
        Blockly.terminateDrag_();
        if (Blockly.selected && Blockly.highlightedConnection_) Blockly.localConnection_.connect(Blockly.highlightedConnection_), b.rendered && (Blockly.localConnection_.isSuperior() ? Blockly.highlightedConnection_ : Blockly.localConnection_).sourceBlock_.connectionUiEffect(), b.workspace.trashcan && b.workspace.trashcan.close();
        else if (!b.getParent() && Blockly.selected.isDeletable() && b.workspace.isDeleteArea(a)) {
            var c =
    ...
    Blockly.doCommand(function() {
        if (!("mousemove" == a.type && 1 >= a.clientX && 0 == a.clientY && 0 == a.button)) {
            ...
            var d = b.getRelativeToSurfaceXY(),
                e = c.moveDrag(a),
                f = b.getSvgRoot();
            1 == Blockly.dragMode_ && goog.math.Coordinate.distance(d, e) * c.scale > Blockly.DRAG_RADIUS && (Blockly.dragMode_ = 2, Blockly.longStop_(), f.translate_ = "", f.skew_ = "", b.parentBlock_ && (b.setParent(null), b.disconnectUiEffect()), b.setDragging_(!0),
                c.recordDeleteAreas());
            if (2 == Blockly.dragMode_) {
                var g = d.x - b.dragStartXY_.x,
                    d = d.y - b.dragStartXY_.y;
                f.translate_ = "translate(" + e.x + "," + e.y + ")";
                f.setAttribute("transform", f.translate_ + f.skew_);
                for (e = 0; e < b.draggedBubbles_.length; e++) f = b.draggedBubbles_[e], f.bubble.setIconLocation(f.x + g, f.y + d);
                for (var f = b.getConnections_(!1), h = null, k = null, l = Blockly.SNAP_RADIUS, e = 0; e < f.length; e++) {
                    var q = f[e],
                        m = q.closest(l, g, d);
                    m.connection && (h = m.connection, k = q, l = m.radius)
                }
                Blockly.highlightedConnection_ && Blockly.highlightedConnection_ !=
                    h && (Blockly.highlightedConnection_.unhighlight(), Blockly.highlightedConnection_ = null, Blockly.localConnection_ = null);
                h && h != Blockly.highlightedConnection_ && (h.highlight(), Blockly.highlightedConnection_ = h, Blockly.localConnection_ = k);
                b.isDeletable() && c.isDeleteArea(a)
            }
        }
        a.stopPropagation()
    })
...
Blockly.BlockSvg.prototype.renderCompute_ = function(a) {
    ...
    for (var d = 0, e = 0, f = !1, g = !1, h = !1, k = void 0, l = this.getInputsInline() && !this.isCollapsed(), q = 0, m; m = b[q]; q++)
        if (m.isVisible()) {
            var p;
            l && k && k != Blockly.NEXT_STATEMENT && m.type != Blockly.NEXT_STATEMENT ? p = c[c.length - 1] : (k = m.type, p = [], p.type = l && m.type !=
                Blockly.NEXT_STATEMENT ? Blockly.BlockSvg.INLINE : m.type, p.height = 0, c.push(p));
            p.push(m);
            ...
            l || q != b.length - 1 ? !l && m.type == Blockly.INPUT_VALUE && b[q + 1] && b[q + 1].type ==
...
Blockly.FieldDropdown.prototype.setText = function(a) {
    this.sourceBlock_ && this.arrow_ && (this.arrow_.style.fill = Blockly.makeColour(this.sourceBlock_.getColour()));
...
Blockly.Flyout.prototype.show = function(a) {
    ...
    this.svgGroup_.style.display =
        "block";
    for (var b = [], g = [], c = 0; e = a[c]; c++) console.log(e), e.tagName && "BLOCK" == e.tagName.toUpperCase() && (d = Blockly.Xml.domToBlock(this.workspace_, e), b.push(d), d = parseInt(e.getAttribute("gap"), 10), g.push(d || 3 * f));
    a = f;
    for (c = 0; d = b[c]; c++) {
        e = d.getDescendants();
        for (var h = 0, k; k = e[h]; h++) k.isInFlyout = !0, k.setCommentText(null);
        console.log(d);
        d.render();
        h = d.getSvgRoot();
        e = d.getHeightWidth();
        ...
        this.listeners_.push(Blockly.bindEvent_(e, "mousedown",
            null, this.createBlockFunc_(d)));
...
Blockly.Toolbox.prototype.init = function() {
    ...
    this.hasColours_ && this.addColour_(b);
    this.position()
};
        ...
        for (var g = 0, h; h = a.childNodes[g]; g++)
            if (h.tagName) switch (h.tagName.toUpperCase()) {
                case "CATEGORY":
                    ...
                    goog.isString(l) ? (k.hexColour = Blockly.makeColour(l), d = !0) : k.hexColour = "";
...
Blockly.Toolbox.prototype.addColour_ = function(a) {
    ...
    for (var b = 0, c; c = a[b]; b++) {
        ...
        if (d) {
            var e = "8px solid " + (c.hexColour || "#ddd");
...
Blockly.Toolbox.prototype.getRect = function() {
    var a = this.workspace_.RTL ? Blockly.svgSize(this.workspace_.options.svg).width - this.width : -1E7;
...
Blockly.Css.inject = function(a, b) {
    if (!Blockly.Css.styleSheet_) {
        ...
        a && (c += Blockly.Css.CONTENT.join("\n"), Blockly.FieldDate && (console.log("FieldData is enable"), c += Blockly.FieldDate.CSS.join("\n")));
...
Blockly.Css.CONTENT = [".blocklySvg {", "background-color: #fff;", "outline: none;", "overflow: hidden;", "}", ".blocklyWidgetDiv {", "display: none;", "position: absolute;", "z-index: 999;", "}", ".blocklyTooltipDiv {", "background-color: #ffffc7;", "border: 1px solid #ddc;", "box-shadow: 4px 4px 20px 1px rgba(0,0,0,.15);", "color: #000;", "display: none;", "font-family: sans-serif;", "font-size: 9pt;", "opacity: 0.9;", "padding: 2px;", "position: absolute;", "z-index: 1000;", "}", ".blocklyResizeSE {", "cursor: se-resize;",
    ...
    "outline: none;", "}", ".blocklyTreeRow {", "height: 22px;", "line-height: 22px;", "margin-bottom: 3px;", "padding-right: 8px;", "white-space: nowrap;", "}", '.blocklyToolboxDiv[dir="RTL"] .blocklyTreeRow {', "margin-left: 8px;", "}", ".blocklyTreeRow:not(.blocklyTreeSelected):hover {", "background-color: #e4e4e4;", "}", ".blocklyTreeSeparator {", "border-bottom: solid #e5e5e5 1px;", "height: 0px;", "margin: 5px 0;", "}", ".blocklyTreeIcon {", "background-image: url(<<<PATH>>>/sprites.png);", "height: 16px;", "vertical-align: middle;",
    "width: 16px;", "}", ".blocklyTreeIconClosedLtr {", "background-position: -32px -1px;", "}", ".blocklyTreeIconClosedRtl {", "background-position: 0px -1px;", "}", ".blocklyTreeIconOpen {", "background-position: -16px -1px;", "}", ".blocklyTreeSelected>.blocklyTreeIconClosedLtr {", "background-position: -32px -17px;", "}", ".blocklyTreeSelected>.blocklyTreeIconClosedRtl {", "background-position: 0px -17px;", "}", ".blocklyTreeSelected>.blocklyTreeIconOpen {", "background-position: -16px -17px;", "}", ".blocklyTreeIconNone,",
    ".blocklyTreeSelected>.blocklyTreeIconNone {", "background-position: -48px -1px;", "}", ".blocklyTreeLabel {", "cursor: default;", "font-family: sans-serif;", "font-size: 16px;", "padding: 0 3px;", "vertical-align: middle;", "}", ".blocklyTreeSelected .blocklyTreeLabel {", "color: #fff;", "}", ".blocklyWidgetDiv .goog-palette {", "outline: none;", "cursor: default;", "}", ".blocklyWidgetDiv .goog-palette-table {", "border: 1px solid #666;", "border-collapse: collapse;", "}", ".blocklyWidgetDiv .goog-palette-cell {", "height: 13px;",
    "width: 15px;", "margin: 0;", "border: 0;", "text-align: center;", "vertical-align: middle;", "border-right: 1px solid #666;", "font-size: 1px;", "}", ".blocklyWidgetDiv .goog-palette-colorswatch {", "position: relative;", "height: 13px;", "width: 15px;", "border: 1px solid #666;", "}", ".blocklyWidgetDiv .goog-palette-cell-hover .goog-palette-colorswatch {", "border: 1px solid #FFF;", "}", ".blocklyWidgetDiv .goog-palette-cell-selected .goog-palette-colorswatch {", "border: 1px solid #000;", "color: #fff;", "}", ".blocklyWidgetDiv .goog-menu {",
...
Blockly.parseOptions_ = function(a) {
    ...
    if (b) var c = null,
        ...
        h = !1,
        k = !1;
    else c = Blockly.parseToolboxTree_(a.toolbox), d = Boolean(c && c.getElementsByTagName("category").length), e = a.trashcan, void 0 === e && (e = d), f = a.collapse, void 0 === f && (f = d), g = a.comments, void 0 === g && (g = d), h = a.disable, void 0 === h && (h = d), k = a.sounds, void 0 === k && (k = !0);
    var l = a.scrollbars;
    void 0 === l && (l = d);
    var q = a.css;
    void 0 === q && (q = !0);
    var m = a.grid || {},
        p = {};
    p.spacing = parseFloat(m.spacing) || 0;
    p.colour = m.colour || "#888";
    p.length = parseFloat(m.length) || 1;
    p.snap = 0 < p.spacing && !!m.snap;
...
Blockly.init_ = function(a) {
    ...
    Blockly.documentEventsBound_ || (Blockly.bindEvent_(document, "keydown", null, Blockly.onKeyDown_), Blockly.bindEvent_(document, "touchend", null, Blockly.longStop_), Blockly.bindEvent_(document, "touchcancel", null, Blockly.longStop_), document.addEventListener("mouseup", Blockly.onMouseUp_, !1),
        goog.userAgent.IPAD && Blockly.bindEvent_(window, "orientationchange", document, function() {
            Blockly.fireUiEvent(window, "resize")
...
Blockly.CHARACTER_SOUP_ = "!#$%()*+,-./:;=?@[]^_`{|}~ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
Blockly.genUid = function() {
    for (var a = [], b = 0; 20 > b; b++) a[b] = Blockly.CHARACTER_SOUP_.charAt(Math.random() * Blockly.CHARACTER_SOUP_.length);
    return a.join("")
};
Blockly.Realtime = {
    isEnabled: function() {
