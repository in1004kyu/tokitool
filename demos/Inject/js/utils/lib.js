...
function rl(){ window.location.reload();};
...
function hitRect(c, pt){
  ...
  if (c.offsetLeft == undefined) return false;
  if (c.offsetTop == undefined) return false;
 	if(x<c.offsetLeft) return false;
	if(x>c.offsetLeft+c.offsetWidth) return false;
	if(y<c.offsetTop) return false;
	if(y>c.offsetTop+c.offsetHeight) return false;
	return true;
}
...
function hit3DRect(c, pt){
  ...
  if (mtx.m41 == undefined) return false;
  if (mtx.m42 == undefined) return false;
 	if(x<mtx.m41) return false;
	if(x>mtx.m41+c.offsetWidth) return false;
	if(y<mtx.m42) return false;
	if(y>mtx.m42+c.offsetHeight) return false;
	return true;
}
...
function hitTest(c, pt){
	...
	if(y<c.offsetTop) return false;
	if(y>c.offsetTop+c.offsetHeight) return false;
	var dx=pt.x-c.offsetLeft, dy=pt.y-c.offsetTop;
...
function CSSTransition(el, obj) {
  ...
  var transition = 'ease';
  var property = 'all';
  var style = {left: el.offsetLeft + 'px', top: el.offsetTop + 'px'};
  var delay = 0;
  var translate = 'translate3d('+ el.offsetLeft+'px,'+el.offsetTop+'px,0)'; // keepit where it is
  var starttime = (new Date()).getTime();
	if (obj.duration) duration = obj.duration;
	if (obj.transition) transition = obj.transition;
	if (obj.property) property = obj.property;
	if (obj.delay) delay = obj.delay;
	if (obj.style) style = obj.style;
	var items = "";
	for (var key in style) items += key +' ' + duration +'s ' + transition + ', ';
...
function CSSTransition3D(el, obj) {
  ...
  var transition = 'ease';
  var property = 'all';
  var style = {left: el.left + 'px', top: el.top + 'px'}; // keepit where it is
  var delay = 0;
  var starttime = (new Date()).getTime();
	if (obj.duration) duration = obj.duration;
	if (obj.transition) transition = obj.transition;
	if (obj.property) property = obj.property;
	if (obj.delay) delay = obj.delay;
	if (obj.style) {for (var key in obj.style) style[key] = obj.style [key];}
...
function getFit(dw, dh){
	if ((dw >= 1) &&(dh >= 1)) return "noscale";
	if ((dw >= 1) &&(dh < 1)) return "scaleh";
	if ((dw< 1) &&(dh >= 1))  return "scalew";
	if (dw < dh) return "scalew";
	return "scaleh";
}
...
function rgb2hsb(str) {
	...
	var rgb = getRGB(num);
	var red=rgb[0]; red/=255;
	var grn=rgb[1]; grn/=255;
	var blu=rgb[2]; blu/=255;	
	min = Math.min(Math.min(red, grn), blu);
	val = Math.max(Math.max(red, grn), blu);
	if (min==val) return new Array (0, 0, val);
...
function rgbToHex(str){
	...
	var	grn=Number (a[1]);
	var	blu=Number (a[2]);
	return rgbToString ({r:  red, g: grn, b: blu });
	}
...
function rgbToString (obj){ return "#" + getHex(obj.r) +  getHex(obj.g) + getHex(obj.b);}
  ...
  for (var i = 0; i < ss.length; ++i) {
  	for (var j = 0; j < ss[i].cssRules.length; ++j) {
  		...
  		for (var k = 0; k < styles.length; ++k) {
	 			if (styles[k].type == window.CSSRule.WEBKIT_KEYFRAMES_RULE && styles[k].name == rule) return styles[k];
    	}
    }
  } // rule not found
  return null;
